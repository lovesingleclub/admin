...
/*****************************************/
//檔案名稱：singleweb_fun20.php
//後台對應位置：約會專家系統/EDM推薦會員
//改版日期：2022.6.15
//改版設計人員：Jack
//改版程式人員：Jack
/*****************************************/
...
if($_REQUEST["st"] == "cancel" && $_REQUEST["t"] != ""){
    $SQL = "update member_data set singleparty_edm=0 where mem_num='".SqlFilter($_REQUEST["t"],"tab")."'";
    $rs = $SPConn->prepare($SQL);
	$rs->execute();
    echo "fix";
    exit();
                    ...
                    <table class="table table-striped table-bordered bootstrap-datatable">
                        <tbody>
                            ...
                            <?php
                                ...
                                if($_REQUEST["sex"] == "2"){
                                    $qsql = " and mem_sex='男'";
                                }elseif($_REQUEST["sex"] == "1"){
                                    $qsql = " and mem_sex='女'";
                                }

                                //取得總筆數
                                $SQL = "Select count(mem_auto) As total_size FROM member_data where mem_level='mem' and singleparty_edm=1 and web_endtime >= getdate()".$qsql;
                                $rs = $SPConn->prepare($SQL);
                                $rs->execute();
                                ...
                                $SQL_list .= "over(order by mem_time desc) As rownumber, * ";
                                $SQL_list .= "From member_data Where mem_level='mem' and singleparty_edm=1".$qsql." and web_endtime >= getdate() ) temp_row ";
                                $SQL_list .= "Where rownumber > ".($tPageSize*$tPage_list);
                                
                                // $SQL = "SELECT * FROM (SELECT TOP " .$page2. " * FROM (SELECT TOP " .($tPageSize*$tPage). " * FROM member_data WHERE mem_level='mem' and singleparty_edm=1".$qsql." and web_endtime >= getdate() order by mem_time desc ) t1 order by mem_time) t2 order by mem_time desc";
