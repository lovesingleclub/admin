...
if request("st") = "send" then
  ...
  strs = request("strs")
  key = request("key")
  vals = ucase(request("vals"))
	c1 = ucase(SHA256(key&strs))
	if c1 = vals then
	response.write "ok"
	else
		response.write "字串:"&strs&"<br>"
		response.write "PD:"&key&"<br>"
		response.write "轉換:"&c1&"<br>"
		response.write "傳回:"&vals&"<br>"
		response.write "no"
	end if
...
字串:<input type="strs" name="strs"><br>
PD:<input type="text" name="key"><br>
比對:<textarea name="vals" cols=50 rows=20></textarea>
<br>
<input type="submit">
</form>