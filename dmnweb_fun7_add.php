<?php 
/*****************************************/
//檔案名稱：dmnweb_fun7_add.php
//後台對應位置：DateMeNow網站系統/新增修改手機版-活動頁面-Banner
//改版日期：2022.8.18
//改版設計人員：Jack
//改版程式人員：Jack
/*****************************************/
...
if ($_REQUEST["st"] == "ups") {
    ...
    $SQL = "UPDATE webdata SET d1='" . $d1 . "' where auton='" . $an . "' and types='index_banner_mobile'";
...
if ($_REQUEST["st"] == "upload") {
    ...
    $SQL = "SELECT top 1 i1 FROM webdata where types='index_banner_mobile' order by i1 desc";
    $rs = $DMNConn->prepare($SQL);
    $rs->execute();
    ...
    $SQL = "SELECT * FROM webdata where auton=".$an." and types='index_banner_mobile'";
    $rs = $DMNConn->prepare($SQL);
    $rs->execute();
    $result = $rs->fetch(PDO::FETCH_ASSOC);
    if ($result) {
        ...
        $SQL = "INSERT INTO webdata (t1, i1, types) VALUES ('" . date("Y/m/d H:i:s") . "','" . $i1 . "','index_banner_mobile')";
        $rs = $DMNConn->prepare($SQL);
        $rs->execute();
    ...
    if ($_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileName = "index_banner_mobile_" . $an . $ext; //檔名
...
if ($_REQUEST["an"] != "") {
    ...
    $SQL = "SELECT * FROM webdata where auton=" . SqlFilter($_REQUEST["an"], "int") . " and types='index_banner_mobile'";
    $rs = $DMNConn->prepare($SQL);
    $rs->execute();
    $result = $rs->fetch(PDO::FETCH_ASSOC);
    if ($result) {
    ...
    <script type="text/javascript">
        $(function() {
            $(".fileupload").each(function() {
                ...
                $this.fileupload({
                        url: "dmnweb_fun7_add.php?st=upload&an=<?php echo SqlFilter($_REQUEST["an"],"int"); ?>",
                        type: "POST",
                        dropZone: $this,
                        dataType: 'html',
                        autoUpload: false,
                        ...
                        add: function(e, data) {
                            ...
                            if (data.files) {
                                if ($("#d1").val().replace(/\s/g, '') == '') {
                                    ...
                                    data.url = "dmnweb_fun7_add.php?st=upload&d1=" + $("#d1").val() + "&an=<?php echo SqlFilter($_REQUEST["an"],"int"); ?>";
                                    data.submit();
                                }
                            }
                        }
