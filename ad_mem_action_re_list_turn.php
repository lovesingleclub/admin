<?php
    ...
    // 退費
    if($_REQUEST["st"] == "payout"){
        $SQL = "SELECT * FROM ac_data_re Where acre_auto = ".SqlFilter($_REQUEST["acre_auto"],"int")."";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $result = $rs->fetch(PDO::FETCH_ASSOC);
        if($result){
            $acre_pay2 = abs(SqlFilter($_REQUEST["acre_pay2"],"int"));
            $acre_pay2 = -1*$acre_pay2;
            $SQL = "UPDATE ac_data_re SET acre_ck2='2', acre_time_del='".date("Y/m/d")."', acre_pay2='".$acre_pay2."' Where acre_auto = ".SqlFilter($_REQUEST["acre_auto"],"int")."";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();
        }
        reURL("win_close.php?m=退費中..");
    }
    ...
    switch($_SESSION["MM_UserAuthorization"]){
        case "admin":
            $sqls2 = "SELECT count(acre_auto) as total_size FROM ac_data_re Where acre_ck2 = 1";
            break;
        case "branch":
        case "action":
        case "love":
        case "pay":
            $sqls2 = "SELECT count(acre_auto) as total_size FROM ac_data_re Where acre_ck2 = 1 and all_branch= '".$_SESSION["branch"]."'";
            break;
    }
    ...
    switch($_SESSION["MM_UserAuthorization"]){
        case "admin":
            $sqls = "SELECT * FROM (SELECT TOP " .$page2. " * FROM (SELECT TOP " .($tPageSize*$tPage). " * FROM ac_data_re Where acre_ck2 = 1";
            break;
        case "branch":
        case "action":
        case "love":
        case "pay":
            $sqls = "SELECT * FROM (SELECT TOP " .$page2. " * FROM (SELECT TOP " .($tPageSize*$tPage). " * FROM ac_data_re Where acre_ck2 = 1 and all_branch= '".$_SESSION["branch"]."'";
            break;
    }
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <span class="title elipsis">
                    <strong>待轉資料查詢 - 數量：<?php echo $total_size; ?></strong> <!-- panel title -->
                </span>
            </div>
            ...
            <div class="panel-body">
                ...
                <div class="col-md-12">
                    <form action="ad_mem_action_re_list_turn.php" method="post" name="form1">
                        <p><span>
                            ...
                            待轉日期： <input type="text" name="times1" id="times1" class="datepicker" autocomplete="off" placeholder="時間區間" value="<?php echo $times1; ?>"> ~ <input type="text" name="times2" id="times2" class="datepicker" autocomplete="off" placeholder="時間區間" value="<?php echo $times2; ?>">
                        </p>
                        <p>
                            會館：
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu pull-right">
                                                    <?php                                            
                                                        ...
                                                        <?php }
                                                    ?>
                                                    <li><a href="javascript:Mars_popup('ad_mem_action_re_turn.php?acre_auto=<?php echo $re["acre_auto"]; ?>','','width=500,height=300,top=200,left=200')"><i class=" icon-share"></i> 轉入活動</a></li>
							                        <li><a href="javascript:Mars_popup('ad_mem_action_re_list_turn.php?st=payout&acre_pay2=<?php echo $re["acre_pay2"]; ?>&acre_auto=<?php echo $re["acre_auto"]; ?>','','width=300,height=200,top=100,left=100')"><i class="icon-refresh"></i> 退費</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
