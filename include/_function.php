...
//申請簽核類型
$sign_option = array();
$sign_option = ["修改身分證字號","修改手機號碼","開秘書後台","關秘書後台","跨區/身分證重覆","再開發資料","其他","活動異動單"];

//會館1
$branch_option = array();
$branch_option = ["台北","桃園","新竹","台中","台南","高雄","八德","約專","迷你約","總管理處","好好玩旅行社"];

//會館2(無迷你約)
$branch2_option = array();
$branch2_option = ["台北","桃園","新竹","台中","台南","高雄","八德","約專","總管理處","好好玩旅行社"];

//會館3(無好好玩)
$branch3_option = array();
$branch3_option = ["台北","桃園","新竹","台中","台南","高雄","八德","約專","迷你約","總管理處"];

//學歷
$school_option = array();
$school_option = ["國中","高中","高職","專科","大學","碩士","博士"];

//年收入
$money_option = array();
$money_option = ["50萬以下","51萬到80萬","81萬到100萬","101萬到199萬","200萬以上"];
...
//for 小組業績表下拉顯示區域轉換(ad_admin_diff_list_team.php)
function changebranch($txt){
	switch ($txt){
		case "台北":
			$etxt = "taipei";
			break;
...
function call_alert_new( $num, $msg, $url ){ //$num=彈跳視窗類型代碼(1)純視窗無轉址；$msg=彈跳訊息；$url=導向路徑
	switch( $num ){
		case 1: //回上頁
			echo "<script language=\"javascript\">" ;
			echo "alert('".$msg."');" ;
...
function daycolor($n){
	if ( $n < 8 ){
		$daycolor = "<font color='#0000ff'>".$n." day</font>";
	}
...
//回報類型
$alloptions = array();
$alloptions = ["--- 已入會 ---","已成交","已是全卡會員","--- 已邀約 ---","已邀約","已邀約本週","已邀約本月","已邀約未到現場","可邀約A","可邀約B","可邀約C","邀約完需再次提醒","--- 需加強 ---","未接","未接4次以上","未接1","未接2","未接3","預約聯絡","有到未參","長期經營","重點經營","到期未續約","需加強推廣活動","需要再排約","已來訪考慮中","--- 排約狀況 ---","已排約","約後關懷","排約未滿5次","排約無效","--- 公開資料 ---","名單資訊","客訴反映","聯繫狀況","排約注意","--- 休息中 ---","請假三個月","請假半年","請假一年","--- 活動咖 ---","課程","喜歡戶外活動","喜歡室內活動","喜歡國外旅遊","--- 重覆 ---","重覆名單","舊會員","--- 無效 ---","黑名單","不要再聯絡","拒絕","暫時拒絕","空號","手機號暫停","已結婚","目前交往中","年紀太小","總管理回收名單","條件不符暫不約","已解約"];
if ( $mem_use == 1 ){
	array_push($alloptions,"--- 會員約見 ---","升卡約見","續約約見,");
}

//排約類別
function invite_types($t){
	switch ($t){
		case "web":
			$invite_types = "春天-自主排約";
			break;
		case "singleparty":
			$invite_types = "約專-會館排約";
			break;
		case "line":
			$invite_types = "LINE排約";
			break;
