...
// 新增
if ($_REQUEST["st"] == "add") {
    $SQL = "SELECT top 1 i1 FROM webdata where types='mobile_index_banner_c' order by i1 desc";
    ...
    $SQL = "INSERT INTO webdata (d1, d2, d3, d4, t1, i1, types) VALUES ('" . SqlFilter($_REQUEST["d1"], "tab") . "','" . $d2 . "','" . SqlFilter($_REQUEST["d3"], "tab") . "','" . SqlFilter($_REQUEST["d4"], "tab") . "','" . date("Y/m/d H:i:s") . "','" . $i1 . "','mobile_index_banner_c')";
    $rs = $SPConn->prepare($SQL);
    $rs->execute();
...
// 更新(有刪除圖檔功能待測)
if ($_REQUEST["st"] == "edit") {
    $SQL = "SELECT * FROM webdata where auton='" . SqlFilter($_REQUEST["an"], "int") . "' and types='mobile_index_banner_c'";
    $rs = $SPConn->prepare($SQL);
    $rs->execute();
    $result = $rs->fetch(PDO::FETCH_ASSOC);
    if ($result) {
        ...
        $SQL = "UPDATE webdata SET d1='" . SqlFilter($_REQUEST["d1"], "tab") . "', d2='" . $d2 . "', d3='" . SqlFilter($_REQUEST["d3"], "tab") . "', d4='" . SqlFilter($_REQUEST["d4"], "tab") . "', t1='" . date("Y/m/d H:i:s") . "' where auton='" . SqlFilter($_REQUEST["an"], "int") . "' and types='mobile_index_banner_c'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
    }
    reURL("win_close.php");
...
if ($_REQUEST["st"] == "upload") {
    ...
    if ($_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileName = "mobile_index_banner_c_" . date("Y") . date("n") . date("j") . date("H") . date("i") . date("s") . rand(1, 99) . "." . $ext; //檔名  
...
<body>
    <p></p>
    <form id="fun_form" method="post" action="springweb_fun23_add.php?st=<?php echo $vst; ?>" onsubmit="return chk_form()">
        <p>
            <label>連結位置：
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $(".fileupload").each(function() {
                ...
                $this.fileupload({
                        url: "springweb_fun23_add.php?st=upload&an=<?php echo SqlFilter($_REQUEST["an"],"int"); ?>",
                        type: "POST",
                        dropZone: $this,
                        dataType: 'html',
                        autoUpload: false,
