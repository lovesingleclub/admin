<?php
    /*****************************************/ 
    //檔案名稱：ad_singleparty_count_ads.php
    //後台對應位置：管理系統/約會專家未入會統計[廣告]
    //改版日期：2022.1.11
    //改版設計人員：Jack
    //改版程式人員：Jack
    /*****************************************/
    ...
    if($_REQUEST["st"] == "send"){
        ...
        if($_REQUEST["start_time"] == $_REQUEST["ostart_time"]){
            ...
            echo "<tr><td>註冊時間</td>";
            echo "<td colspan=2>Facebook</td>";
            echo "<td colspan=2>Google</td>";
            echo "<td colspan=2>Yahoo</td>";
            echo "<td colspan=2>Line</td>";
            echo "<td colspan=2>Youtube</td>";
            echo "<td colspan=2>accupass</td>";
            echo "<td colspan=2>affiliates</td>";
            echo "<td colspan=2>cheers</td>";
            echo "<td colspan=2>wordpress</td>";
            echo "<td colspan=2>EDM</td>";
            echo "<td>廣告註冊總數</td>";
            // echo "<td>首次造訪</td>";
        ...
        $all_str = "facebook|1,google|2,yahoo|3,line|4,youtube|5,accupass|6,affiliates|7,cheers|8,wordpress|9,EDM|10";
        $allnew = 0;
        $allsize = 0;
        $allstrsize = 0;
        ...
        foreach(explode(",",$all_str) as $pp){
            ...
            $SQL = "SELECT count(mem_auto) as tt FROM member_data as dba Where mem_level='guest' and mem_come='約會專家' and datediff(d, mem_time, '".$showdate."') = 0".$vsql;
            ...
            $SQL = "SELECT count(mem_auto) as tt FROM member_data as dba Where mem_level='guest' and mem_come='約會專家' and datediff(d, mem_time, '".$showdate."') = 0".$vsql." And ((SELECT count(mem_auto) FROM member_data Where mem_mobile = dba.mem_mobile and datediff(s, dba.mem_time, mem_time) <= 0) <= 1) And ((SELECT count(k_id) FROM love_keyin Where  k_mobile = dba.mem_mobile) <= 0)";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();
            $result = $rs->fetch(PDO::FETCH_ASSOC);
            if($result){
        ...
        $SQL = "SELECT count(mem_auto) as tt FROM member_data as dba Where mem_level='guest' and mem_come='約會專家' and datediff(d, mem_time, '".$showdate."') = 0 and mem_cc in (".$all_str2.")";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $result = $rs->fetch(PDO::FETCH_ASSOC);
        if ($result) {
        ...
        for($i=1;$i<=$allstrsize;$i++){
            ...
            echo "<td>".${"t".$i}."</td><td>".${"t".$i."a"}."</td>"; //facebook 
        }
...
<section id="middle">
    ...
    <header id="page-header">
        <ol class="breadcrumb">
            ...
            <li class="active">約會專家未入會統計[廣告]</li>
        </ol>
    </header>
    <!-- /page title -->
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <span class="title elipsis">
                    <strong>約會專家未入會統計[廣告]</strong> <!-- panel title -->
                </span>
            </div>
...
<script type="text/javascript">
    ...
    function conutice_ajax(n1, n2, n3, n4) {
        setTimeout(function() {
            $.ajax({
                url: 'ad_singleparty_count_ads.php?st=send',
                data: {
                    start_time: n1,
                    ostart_time: n2,
                    end_time: n3,
