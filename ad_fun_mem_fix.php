            ...
            <td width="650" valign="top">
                <form action="ad_fun_mem_fix.php?state=add" method="post" name="form5" onSubmit="VF_form5();return false;">
                    <table width="650" border="1">
                        ...
                        <tr>
                            <td>處理單位：<?php echo $result["mem_branch"];?></td>
                        </tr>
                        <tr>
                            <td>帳號：
                                <input name="mem_username" type="text" id="mem_username" style="font-size: 9pt" onBlur="NM_changeCase('mem_username',0);this.value=toZenkakuNum(this.value)" value="<?php echo $result["mem_username"];?>" size="15" maxlength="10">
                                <font color="#999999">（ 請填入<font color="#FF0000">身分證字號</font>）</font>密碼：<font color="#999999">
                                    <input name="mem_passwd" type="text" id="mem_passwd" style="font-size: 9pt" value="<?php echo $result["mem_passwd"];?>" size="15" maxlength="8">
                        ...
                        <tr>
                            <td>E-mail：
                                ...
                                　MSN/LINE：
                                <input name="mem_msn" type="text" id="mem_msn" style="font-size: 9pt" value="<?php echo $result["mem_msn"]; ?>" size="35">
                            </td>
                        </tr>
                        <tr>
                            <td>地區：<font size="2">
                        ...
                        <tr>
                            <td>身高：
                                <input name="mem_he" type="text" id="mem_he2" value="<?php echo $result["mem_he"]; ?>" size="5">
                                公分　體重：
                                <input name="mem_we" type="text" id="mem_we2" style="font-size: 9pt" value="<?php echo $result["mem_we"]; ?>" size="5">
                                公斤
                            </td>
                        </tr>
                        <tr>
                            <td>血型：
                                <?php 
                                    ...
                                    foreach($blood as $bl){
                                        if($bl == $result["mem_blood"]){
                                            echo "<input type='radio' name='mem_blood' value='".$bl."' checked>".$bl."型 ";
                                        }else{
                                            echo "<input type='radio' name='mem_blood' value='".$bl."'>".$bl."型 ";
                                        }
                                    }
                                ?>
                            </td>
                        ...
                        <tr>
                            <td>職業：<font size="2">
                                    <select name="mem_job1" id="mem_job1" style="font-size: 9pt">
                                        <?php
                                            $job1 = array("公務/國家機關","司法/法務","軍警單位","自營/投資","電腦/網路","電子/通信/電器","教育/研究單位","醫療/護理服務業","媒體傳播/出版業","藝術/音樂/設計","建築/裝修/物業","營銷/業務",
...
// end_of_saved_settings
    ...
    for(var r8=0;r8<theForm['mem_sex'].length;r8++){
        if(theForm['mem_sex'][r8].checked)rFlg_mem_sex=true;
    }
    for(var r6=0;r6<theForm['mem_school'].length;r6++){
        if(theForm['mem_school'][r6].checked)rFlg_mem_school=true;
    }
    for(var r5=0;r5<theForm['mem_blood'].length;r5++){
        if(theForm['mem_blood'][r5].checked)rFlg_mem_blood=true;
    }
    for(var s4=0;s4<theForm['mem_star'].length;s4++){
        if(theForm['mem_star'].options[s4].selected){
            if(theForm['mem_star'].options[s4].text==theForm['mem_star'].options[0].text){
                sFlg_mem_star=true;
            }   
        }
    }
    for(var s0=0;s0<theForm['mem_area'].length;s0++){
        if(theForm['mem_area'].options[s0].selected){
            if(theForm['mem_area'].options[s0].text==theForm['mem_area'].options[0].text){
                sFlg_mem_area=true;
            }            
        }
    }
    if (!rFlg_mem_school){ 
        errMsg="學歷沒有選擇" ; setfocus="['mem_school'][0]" ; 
    }
    if (!rFlg_mem_blood){ 
        errMsg="血型沒有選擇" ; setfocus="['mem_blood'][0]" ;
    }
    if (sFlg_mem_star){
        errMsg="星座沒有選擇" ; setfocus="['mem_star']";
    }
    if (theForm['mem_nick'].value=="" ){ 
        errMsg="暱稱沒有填" ; setfocus="['mem_nick']" ; 
    }
    if (sFlg_mem_area){ 
        errMsg="地區沒有選擇" ; setfocus="['mem_area']" ;
    }
    if (theForm['mem_address'].value=="" ){
        errMsg="地址沒有填" ; setfocus="['mem_address']" ;
    }
    if (!rFlg_mem_sex){
        errMsg="性別沒有選擇" ; setfocus="['mem_sex'][0]" ;
    }
    if (theForm['mem_name'].value=="" ){
        errMsg="姓名沒有填" ; setfocus="['mem_name']" ;
    }
    if (errMsg !="" ){
        alert(errMsg); eval("theForm" + setfocus + ".focus()" );
    } else {
        theForm.submit();
    }
    ...
    function toZenkakuNum(motoText){
        ...
        for (i=0; i<motoText.length; i++) {
            c=motoText.charAt(i); n=zen.indexOf(c,0); if (n>= 0) c = han.charAt(n);
            str += c;
        }
        return str;
    }
