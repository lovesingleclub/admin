...
// 刪除圖片(待測試)
if($_REQUEST["st"] == "del"){
	$SQL = "select d2 from si_webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='event_banner_pc'";
	$rs = $SPConn->prepare($SQL);
	$rs->execute();
	$result = $rs->fetch(PDO::FETCH_ASSOC);
	if($result){
		DelFile(("singleparty_image/event/".$result["d2"]));
		$SQL = "delete from si_webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='event_banner_pc'";
		$rs = $SPConn->prepare($SQL);
		$rs->execute();
		if($rs){
			reURL("singleweb_fun7.php");
...
<section id="middle">
	...
	<header id="page-header">
		<ol class="breadcrumb">
			...
			<li>約會專家系統</li>
			<li class="active">主題約會-Banner 電腦版</li>
		</ol>
	</header>
	<!-- /page title -->
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<span class="title elipsis">
					<strong>主題約會-Banner 電腦版</strong> <!-- panel title -->
				</span>
			</div>

			<div class="panel-body">
				<p><input type="button" class="btn btn-info" value="新增 Banner" onclick="Mars_popup('singleweb_fun7_add.php','','scrollbars=yes,status=yes,menubar=yes,resizable=yes,width=690,height=300,top=10,left=10');"></p>
				<table class="table table-striped table-bordered bootstrap-datatable">
					<tbody>
						...
						<?php 
                            $SQL = "SELECT * FROM si_webdata where types='event_banner_pc' order by i1 desc";
                            $rs = $SPConn->prepare($SQL);
                            $rs->execute();
                            ...
                            if($result){
                                ...
                                foreach($result as $re){
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:Mars_popup('singleweb_fun7_add.php?an=<?php echo $re["auton"] ?>','','scrollbars=yes,status=yes,menubar=yes,resizable=yes,width=690,height=300,top=10,left=10');">編輯</a>
                                            <a title="刪除" href="singleweb_fun7.php?st=del&an=<?php echo $re["auton"] ?>">刪除</a>						
                                        </td>
                                    </tr>
                                <?php $ii = $ii+1; }   
                            }else{
