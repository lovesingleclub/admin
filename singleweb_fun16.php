...
/*****************************************/
//檔案名稱：singleweb_fun16.php
//後台對應位置：約會專家系統/主題活動管理
//改版日期：2022.5.31
//改版設計人員：Jack
//改版程式人員：Jack
/*****************************************/
...
// 刪除圖片(待測試)
if($_REQUEST["st"] == "del"){
	$SQL = "select d2, d3 from si_webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='event_custom'";
	$rs = $SPConn->prepare($SQL);
	$rs->execute();
	$result = $rs->fetch(PDO::FETCH_ASSOC);
	if($result){
		DelFile(("singleparty_image/event_custom/".$result["d2"]));
        DelFile(("singleparty_image/event_custom/".$result["d3"]));
		$SQL = "delete from si_webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='event_custom'";
		$rs = $SPConn->prepare($SQL);
		$rs->execute();
		if($rs){
			reURL("singleweb_fun16.php");
            ...
            <div class="panel-body">
                ...
                <table class="table table-striped table-bordered bootstrap-datatable">
                    <tbody>
                        ...
                        <?php 
                            $SQL = "SELECT * FROM si_webdata where types='event_custom' order by i1 desc";
                            $rs = $SPConn->prepare($SQL);
                            $rs->execute();
                                ...
                                foreach($result as $re){
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php 
                                                if($re["d5"] != ""){ ?>
                                                    <a href="singleparty_image/event_custom/<?php echo $re["d5"] ?>" class="fancybox"><img src="singleparty_image/event_custom/<?php echo $re["d5"] ?>" border=0 height=40></a>
                                                <?php }
                                            ?>
                                        </td>
                                        <td><?php echo $re["d1"] ?></td>
                                        <td><?php echo $re["n1"] ?></td>
                                        <td><?php echo $re["d4"] ?></td>
                                        <td>
                                            <a href="singleweb_fun16_add.php?an=<?php echo $re["auton"] ?>">編輯</a>
                                            <a title="刪除" href="singleweb_fun16.php?st=del&an=<?php echo $re["auton"] ?>">刪除</a>						
                                        </td>
                                    </tr>
                                <?php $ii = $ii+1; }   
                            }else{
