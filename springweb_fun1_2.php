<?php
    ...
    if($_REQUEST["st"] == "mup"){
        ...
        $upline = $nowline+1;
        $SQL = "update webdata set i1=".$nowline." where i1='".$upline."' and types='index_banner_top'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $SQL = "update webdata set i1=".$upline." where auton=".SqlFilter($_REQUEST["an"],"int")." and types='index_banner_top'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();

        reURL("springweb_fun1_2.php");
    }
    ...
    // 刪除圖片(待測試)
    if($_REQUEST["st"] == "del"){
        $SQL = "select d2 from webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='index_banner_top'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $result = $rs->fetch(PDO::FETCH_ASSOC);
        if($result){
            DelFile(("upload_image/".$result["d2"]));
            $SQL = "delete from webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='index_banner_top'";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();
            if($rs){
                reURL("springweb_fun1_2.php");
            ...
            <div class="panel-body">
                ...
                <table class="table table-striped table-bordered bootstrap-datatable">
                    <tbody>
                        ...
                        <?php 
                            $SQL = "SELECT * FROM webdata where types='index_banner_top' order by i1 desc";
                            $rs = $SPConn->prepare($SQL);
                            $rs->execute();
                            ...
                            if($result){
                                ...
                                foreach($result as $re){
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:Mars_popup('springweb_fun1_2_add.php?an=<?php echo $re["auton"] ?>','','scrollbars=yes,status=yes,menubar=yes,resizable=yes,width=690,height=300,top=10,left=10');">編輯</a>
                                            <a title="刪除" href="springweb_fun1_2.php?st=del&an=<?php echo $re["auton"] ?>">刪除</a>						
                                        </td>
                                    </tr>
                                <?php $ii = $ii+1; }   
                            }else{
