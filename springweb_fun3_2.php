<?php
    ...
    if($_REQUEST["st"] == "mup"){
        ...
        $upline = $nowline+1;
        $SQL = "update webdata set i1=".$nowline." where i1='".$upline."' and types='witness_banner'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $SQL = "update webdata set i1=".$upline." where auton=".SqlFilter($_REQUEST["an"],"int")." and types='witness_banner'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();

        reURL("springweb_fun3_2.php");
    }
    ...
    // 刪除圖片(待測試)
    if($_REQUEST["st"] == "del"){
        $SQL = "select d2 from webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='witness_banner'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $result = $rs->fetch(PDO::FETCH_ASSOC);
        if($result){
            DelFile(("upload_image/".$result["d2"]));
            $SQL = "delete from webdata where auton=".SqlFilter($_REQUEST["an"],"int")." and types='witness_banner'";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();
            if($rs){
                reURL("springweb_fun3_2.php");
            ...
            <div class="panel-body">
                ...
                <table class="table table-striped table-bordered bootstrap-datatable">
                    <tbody>
                        ...
                        <?php 
                            $SQL = "SELECT * FROM webdata where types='witness_banner' order by i1 desc";
                            $rs = $SPConn->prepare($SQL);
                            $rs->execute();
