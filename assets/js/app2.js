...
function _ajaxmodal() {	
	if(jQuery("#ajax_load_modal").length > 0) {		
		$("#ajax_load_modal").on("show.bs.modal", function(e) {
    var $link = $(e.relatedTarget);   
    $(this).find(".modal-load").load($link.attr("href"));
    });	
	}
}
...
function _checkreservation() {
		var $reservation_alert = _getCookie("reservation_alert"), $nexttime, $nexttime_diff = 0, $nexttime_diff_mi = 0;
	if($reservation_alert) {
		//console.log($reservation_alert);	
	  $.each( $reservation_alert.split("|o|"), function( $key, $value ) {
	  	$rtime = $value.split("|a|")[0];
	  	$rtime = $rtime.replace("|s|", " ");
	  	$rtime = new Date($rtime);	  	
	  	$nowtime   = new Date();	
	  	$diff  = ($rtime-$nowtime) / 1000;
	  	if($diff > 0) {
	  		if(!$nexttime_diff) {
	  			$nexttime = $value;
	  			$nexttime_diff = $diff;
	  			$nexttime_diff_mi = $nexttime_diff / 60;
	  		} else if($diff <= $nexttime_diff) {
	  			$nexttime = $value;
	  			$nexttime_diff = $diff;
	  			$nexttime_diff_mi = $nexttime_diff / 60;
	  		}
      ...
      if($isshowtime != $nexttime_diff_mi && ($nexttime_diff_mi == 1 || $nexttime_diff_mi == 5)) {  //�p�G�O 1, 5 ���~����
       setTimeout(function(){
	       $reservation_alert_modal_time = $nexttime.split("|a|")[0].replace("|s|", " ");	
	       $reservation_alert_modal_us = $nexttime.split("|a|")[1];
	       $("#reservation_alert_modal").modal({backdrop:"static",keyboard:true});
	       $("#reservation_alert_modal").on("shown.bs.modal", function() {
         $(this).find("#reservation_alert_modal_mi").html($nexttime_diff_mi); 
         $(this).find("#reservation_alert_modal_time").html($reservation_alert_modal_time); 
         $(this).find("#reservation_alert_modal_us").html($reservation_alert_modal_us);     
         });	      
       }, 500);

       $isshowtime = $nexttime_diff_mi;
      }
...
function _mtu() {
	if(jQuery("select").length) {
		...
		var $thissel = $(this);
		if($thissel.parents("form").length) {
			var $thisform = $thissel.parents("form");
			if($thisform.hasClass("form-inline") && !$thissel.hasClass("form-control") && !$thissel.hasClass("form-control2")) $thissel.addClass("form-control");
		}
	  });
	}
}
 ...
 **************************************************************** **/
	function _lightbox() {
		...
		var _el2 = jQuery(".fancybox");
		if(_el.length > 0 || _el2.length > 0) {
			loadScript(plugin_path + 'magnific-popup/jquery.magnific-popup.min.js', function() {
				...
				_el.each(function() {
					...
					var _t 			= jQuery(this),
						...
						mfpmw 	= _t.attr('data-lightbox-mw'),
						config		= {},
						defaults 	= {
							type: 				'image',
							...
							callbacks: {								
							  resize: function() {
                 if(jQuery.isNumeric(mfpmw)) $(".mfp-content").css("width", mfpmw+"%");
                 else $(".mfp-content").css("width", "auto").addClass("padding-left-10").addClass("padding-right-10");
                }
              },
 ...
 **************************************************************** **/
	function _pickers() {
		...
		if(_container_1.length > 0) {
			loadScript(plugin_path + 'bootstrap.datepicker/js/bootstrap-datepicker.min.js?v=1.2', function() {
		
				if(jQuery().datepicker) {
					_container_1.each(function() {
						var _t 		= jQuery(this),
							_lang 	=	_t.attr('data-lang') || 'tw';

						if(_lang != 'en' && _lang != '') { // load language file
							loadScript(plugin_path + 'bootstrap.datepicker/locales/bootstrap-datepicker.'+_lang+'.min.js?v=1.2');
						}
						...
						jQuery(this).datepicker({
							...
							calendarWeeks: 	_t.attr('data-calendarWeeks') 	== "false" ? true : false,
