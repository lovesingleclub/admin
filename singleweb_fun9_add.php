<?php 
    /*****************************************/
    //檔案名稱：singleweb_fun9_add.php
    //後台對應位置：約會專家系統/首頁-Banner 電腦版>新增BANNER
    //改版日期：2022.5.18
    //改版設計人員：Jack
    //改版程式人員：Jack
    /*****************************************/
    ...
    // 新增
    if($_REQUEST["st"] == "add"){
        $SQL = "SELECT top 1 i1 FROM si_webdata where types='index_banner' order by i1 desc";
        ...
        $SQL = "INSERT INTO si_webdata (d1, d2, alt, d3, d4, t1, i1, types) VALUES ('".SqlFilter($_REQUEST["d1"],"tab")."','".$d2."','".SqlFilter($_REQUEST["alt"],"tab")."','".SqlFilter($_REQUEST["d3"],"tab")."','".SqlFilter($_REQUEST["d4"],"tab")."','".date("Y/m/d H:i:s")."','".$i1."','index_banner')";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
    ...
    // 更新(有刪除圖檔功能待測)
    if($_REQUEST["st"] == "edit"){
        $SQL = "SELECT * FROM si_webdata where auton='".SqlFilter($_REQUEST["an"],"int")."' and types='index_banner'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $result = $rs->fetch(PDO::FETCH_ASSOC);
        if($result){            
            ...
            $SQL = "UPDATE si_webdata SET d1='".SqlFilter($_REQUEST["d1"],"tab")."', d2='".$d2."', alt='".SqlFilter($_REQUEST["alt"],"tab")."', d3='".SqlFilter($_REQUEST["d3"],"tab")."', d4='".SqlFilter($_REQUEST["d4"],"tab")."', t1='".date("Y/m/d H:i:s")."' where auton='".SqlFilter($_REQUEST["an"],"int")."' and types='index_banner'";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();          
        }
        reURL("win_close.php");
    ...
    if($_REQUEST["st"] == "upload"){
        if ($_FILES['fileupload']['error'] === UPLOAD_ERR_OK){
            ...
            $fileName = "index_banner_".date("Y").date("n").date("j").date("H").date("i").date("s").rand(1,99).".".$ext; //檔名
...
<body>
    ...
    <p></p>
    <form id="fun_form" method="post" action="singleweb_fun9_add.php?st=<?php echo $vst; ?>" onsubmit="return chk_form()">
        <p>
            <label>連結位置：
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $(".fileupload").each(function() {
                ...
                $this.fileupload({
                        url: "singleweb_fun9_add.php?st=upload&an=<?php echo SqlFilter($_REQUEST["an"],"int"); ?>",
                        type: "POST",
                        dropZone: $this,
                        dataType: 'html',
                        autoUpload: false,
