<?php
    ...
    if($_REQUEST["st"] == "up_line"){
        ...
        $upline = $nowline+1;
        $SQL = "update ad_enterprise set adc_desc=".$nowline." where adc_desc=".$upline."";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $SQL = "update ad_enterprise set adc_desc=".$upline." where adc_auto=".SqlFilter($_REQUEST["adc_auto"],"int")."";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
    }
    ...
    // 刪除(有刪除圖片功能待測)
    if($_REQUEST["st"] == "del"){
        $SQL = "select * from web_photo where num=".SqlFilter($_REQUEST["id"],"int")." and types='enterprise'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        ...
        $SQL = "delete from web_photo where num=".SqlFilter($_REQUEST["id"],"int")." and types='enterprise'";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
        $SQL = "delete from ad_enterprise where adc_auto=".SqlFilter($_REQUEST["id"],"int")."";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
            ...
            <div class="panel-body">
                ...
                <table class="table table-striped table-bordered bootstrap-datatable">
                    <tbody>
                        ...
                        <?php 
                            $SQL = "SELECT * FROM ad_enterprise order by adc_desc desc";
                            $rs = $SPConn->prepare($SQL);
                            $rs->execute();
                            $result = $rs->fetchAll(PDO::FETCH_ASSOC);
                            if($result){
                                ...
                                foreach($result as $re){
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="springweb_fun13_add.php?act=up&id=<?php echo $re["adc_auto"]; ?>">編輯</a>					
                                            <a href="javascript:Mars_popup2('springweb_fun13.php?st=del&id=<?php echo $re["adc_auto"]; ?>','','width=300,height=200,top=100,left=100')">刪除</a>						
                                        </td>
                                    </tr>
                                <?php $ii = $ii+1; }
                            }else{
