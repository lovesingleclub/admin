<?php 
/*****************************************/
//檔案名稱：dmnweb_fun8_add.php
//後台對應位置：DateMeNow網站系統/新增手機版-首頁-Banner
//改版日期：2022.8.17
//改版設計人員：Jack
//改版程式人員：Jack
/*****************************************/
...
// 新增
if ($_REQUEST["st"] == "add") {
    $SQL = "SELECT top 1 i1 FROM webdata where types='newindex_banner_mobile' order by i1 desc";
    ...
    $SQL = "INSERT INTO webdata (d1, d2, alt, d3, d4, t1, i1, types) VALUES ('" . SqlFilter($_REQUEST["d1"], "tab") . "','" . $d2 . "','" . SqlFilter($_REQUEST["alt"], "tab") . "','" . SqlFilter($_REQUEST["d3"], "tab") . "','" . SqlFilter($_REQUEST["d4"], "tab") . "','" . date("Y/m/d H:i:s") . "','" . $i1 . "','newindex_banner_mobile')";
    $rs = $DMNConn->prepare($SQL);
    $rs->execute();
...
// 更新(有刪除圖檔功能待測)
if ($_REQUEST["st"] == "edit") {
    $SQL = "SELECT * FROM webdata where auton='" . SqlFilter($_REQUEST["an"], "int") . "' and types='newindex_banner_mobile'";
    $rs = $DMNConn->prepare($SQL);
    $rs->execute();
    $result = $rs->fetch(PDO::FETCH_ASSOC);
    if ($result) {
        ...
        $SQL = "UPDATE webdata SET d1='" . SqlFilter($_REQUEST["d1"], "tab") . "', d2='" . $d2 . "', alt='" . SqlFilter($_REQUEST["alt"], "tab") . "', d3='" . SqlFilter($_REQUEST["d3"], "tab") . "', d4='" . SqlFilter($_REQUEST["d4"], "tab") . "', t1='" . date("Y/m/d H:i:s") . "' where auton='" . SqlFilter($_REQUEST["an"], "int") . "' and types='newindex_banner_mobile'";
        $rs = $DMNConn->prepare($SQL);
        $rs->execute();
    }
    reURL("win_close.php");
...
if ($_REQUEST["st"] == "upload") {
    if ($_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileName = "newindex_banner_mobile_" . date("Y") . date("n") . date("j") . date("H") . date("i") . date("s") . rand(1, 99) . "." . $ext; //檔名
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $(".fileupload").each(function() {
                ...
                $this.fileupload({
                        url: "dmnweb_fun8_add.php?st=upload&an=<?php echo SqlFilter($_REQUEST["an"],"int"); ?>",
                        type: "POST",
                        dropZone: $this,
                        dataType: 'html',
                        autoUpload: false,
