<?php
    /*****************************************/ 
    //檔案名稱：ad_counts_ads.php
    //後台對應位置：管理系統/未入會統計[廣告]
    //改版日期：2022.1.6
    //改版設計人員：Jack
    //改版程式人員：Jack
    /*****************************************/
    ...
    if($_REQUEST["st"] == "send"){
        ...
        if($_REQUEST["start_time"] == $_REQUEST["ostart_time"]){
            ...
            echo "<td colspan=4>niunews_pay</td>";
            echo "<td colspan=4>niusnews_banner_tks</td>";
            echo "<td>廣告註冊總數</td>";
            // echo "<td>首次造訪</td>";
        ...
        $all_str = "facebook|1,google|2,yahoo|3,line|4,youtube|5,accupass|6,affiliates|7,cheers|8,wordpress|9,EDM|10,Google_GDN|11,Google_GSP|12,thenewslens|13,niunews_pay|14,niusnews_banner_tks|15";
        $allnew = 0;
        $allsize = 0;
        $allstrsize = 0;
        ...
        foreach(explode(",",$all_str) as $pp){
            ...
            $SQL = "SELECT count(mem_auto) as tt FROM member_data as dba Where mem_level='guest' and mem_sex='男' and mem_branch <> '八德' and datediff(d, mem_time, '".$showdate."') = 0".$vsql." And ((SELECT count(mem_auto) FROM member_data Where mem_branch <> '八德' and mem_mobile = dba.mem_mobile and datediff(s, dba.mem_time, mem_time) <= 0) <= 1) And ((SELECT count(k_id) FROM love_keyin Where all_branch <> '八德' and k_mobile = dba.mem_mobile) <= 0)";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();
            $result = $rs->fetch(PDO::FETCH_ASSOC);
            if($result){
            ...
            $SQL = "SELECT count(mem_auto) as tt FROM member_data as dba Where mem_level='guest' and mem_sex='女' and mem_branch <> '八德' and datediff(d, mem_time, '".$showdate."') = 0".$vsql." And ((SELECT count(mem_auto) FROM member_data Where mem_branch <> '八德' and mem_mobile = dba.mem_mobile and datediff(s, dba.mem_time, mem_time) <= 0) <= 1) And ((SELECT count(k_id) FROM love_keyin Where all_branch <> '八德' and k_mobile = dba.mem_mobile) <= 0)";
            $rs = $SPConn->prepare($SQL);
            $rs->execute();
            $result = $rs->fetch(PDO::FETCH_ASSOC);
            if($result){
