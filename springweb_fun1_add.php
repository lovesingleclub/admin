<?php 
    /*****************************************/
    //檔案名稱：springweb_fun1_add.php
    //後台對應位置：春天網站系統/首頁-中間BANNER>新增banner
    //改版日期：2022.4.2
    //改版設計人員：Jack
    //改版程式人員：Jack
    /*****************************************/
    ...
    if($_REQUEST["st"] == "add"){
        ...
        $SQL = "INSERT INTO webdata (d1, d2, alt, d3, d4, t1, i1, types) VALUES ('".SqlFilter($_REQUEST["d1"],"tab")."','".$d2."','".SqlFilter($_REQUEST["alt"],"tab")."','".SqlFilter($_REQUEST["d3"],"tab")."','".SqlFilter($_REQUEST["d4"],"tab")."','".date("Y/m/d H:i:s")."','".$i1."','index_banner')";
        $rs = $SPConn->prepare($SQL);
        $rs->execute();
...
<body>
    <p></p>
    <form id="fun_form" method="post" action="springweb_fun1_add.php?st=<?php echo $vst ?>" onsubmit="return chk_form()">
        <p>
            <label>連結位置：
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $(".fileupload").each(function() {
                ...
                $this.fileupload({
                        url: "springweb_fun1_add.php?st=upload&an=<?php echo SqlFilter($_REQUEST["an"],"int"); ?>",
                        type: "POST",
                        dropZone: $this,
                        dataType: 'html',
                        autoUpload: false,
